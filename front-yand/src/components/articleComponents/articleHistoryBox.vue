...
<script>
import {getArticleHistory} from "@/config/ApiConfig/articleApiConfig/articleApiConfig";
import {getUserAccountbyID} from "@/config/ApiConfig/userApiConfig/userApiConfig";

export default {
  name: "articleHistoryBox",
  props:{
    ArticleId:null
  },
  data(){
  ...
  methods:{
    edit(item){
      ...
      xhr.open('GET', item.articleUrl,true);
      xhr.send(null);
      ...
      this.$parent.$parent.$parent.openHistory=false
    }
  },
  mounted() {
    this.axios.get(getArticleHistory+this.ArticleId)
        .then(res => {
          if (!res.data.code) {
